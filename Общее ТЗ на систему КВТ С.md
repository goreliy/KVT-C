...
> Poller предоставляет отдельную страницу сервиса (`GET /` на порту Poller) для полного управления: старт/стоп, статус, лог, отдельные окна TX/RX Modbus в HEX-байтах, настройка `device_slave_id` и поиск устройств по Slave ID.
