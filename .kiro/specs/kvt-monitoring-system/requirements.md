...
14. Modbus_Poller ДОЛЖЕН иметь собственную веб-страницу сервиса на порту Poller (`GET /`) с полным интерфейсом управления: просмотр статуса, просмотр лога, отдельные окна TX/RX Modbus в байтовом (HEX) виде, изменение `device_slave_id`, запуск/остановка опроса и запуск сканирования устройств.
