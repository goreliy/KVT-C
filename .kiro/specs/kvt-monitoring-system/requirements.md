...
8. Web_Visualizer ДОЛЖЕН предоставлять на странице `/settings/poller` переключатель `use_mock_server` для тестового режима, кнопки запуска/остановки Mock Server и кнопку перехода на страницу Mock Server.
9. КОГДА оператор нажимает запуск Mock Server из основной визуализации, система ДОЛЖНА запустить тестовый Mock Server и вернуть его статус; КОГДА оператор нажимает остановку — система ДОЛЖНА остановить Mock Server и обновить статус.
10. КОГДА включён `use_mock_server`, Visualizer ДОЛЖЕН сохранять параметры подключения к Mock Server (`mock_server_url`) в `poller_config.json` для использования в тестовом контуре без изменения протокола Modbus Poller.
11. Modbus_Poller продолжает работать только по Modbus RTU через COM-порт; использование Mock Server в данном ТЗ относится к тестовому контуру визуализации и отладки, а не к переходу Poller на Modbus TCP.
